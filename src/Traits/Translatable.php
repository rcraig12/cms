...
namespace RCS\CMS\Traits;

use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\Relations\Relation;
use RCS\CMS\Models\Translation;
use RCS\CMS\Translator;
...
trait Translatable
{
    ...
    public function translations()
    {
        return $this->hasMany(Translation::class, 'foreign_key', $this->getKeyName())
            ...
            ->whereIn('locale', config('cms.multilingual.locales', []));
    }
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('cms.multilingual.locales', [config('cms.multilingual.default')]);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('cms.multilingual.default'), false];
        }
        ...
        $default = config('cms.multilingual.default');
    ...
    public function setAttributeTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('cms.multilingual.default', 'en');
        $locales = config('cms.multilingual.locales', [$default]);
    ...
    public function prepareTranslations(&$request)
    {
        ...
        foreach ($transFields as $field) {
            ...
            $request->merge([$field => $trans[config('cms.multilingual.default', 'en')]]);
